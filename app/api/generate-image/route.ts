  ...
  try {
    ...
    const { data: jobData, error: jobError } = await supabase
      ...
      .insert({
        ...
        quality,
        status: "pending",
        image_url: null, // NULL inicialmente
    ...
    if (jobError || !jobData) {
      ...
      return NextResponse.json(
        {
          error: "Erro ao criar job de imagem",
          details: jobError?.message || "Erro desconhecido",
          code: jobError?.code || "UNKNOWN",
          hint: jobError?.hint || "Verifique as permissões do banco de dados",
        },
        { status: 500 },
      )
    }
    ...
    return NextResponse.json({
      success: true,
      jobId: jobData.id,
      message: "Geração de imagem iniciada com sucesso",
